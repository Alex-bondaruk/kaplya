...
final class MSMReplicatorEngine
{
	...
	public function enqueue_for_replication( MSMMediaItem $media_item ) {
		...
		if( $media_item->is_ignored() ){
			...
			$unfiltered_media_item = $media_item;
			$media_item = apply_filters_deprecated(
				'msm_filter_replicable_attachments',
				array( $media_item ),
				'1.3.0',
				'msm_media_item_is_ignored',
				'Note changed parameters with \'msm_media_item_is_ignored\''
			);

			// If some filter marks media_item "empty", consider item as ignored
			// @since 1.3
			if( empty( $media_item ) ){
				$unfiltered_media_item->mark_ignored();
			}
		}
