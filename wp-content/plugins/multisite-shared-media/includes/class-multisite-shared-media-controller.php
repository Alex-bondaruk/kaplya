...
class MSMController {
	...
	public function rewrite_upload_dir( $dirs ) {
		...
		if( defined( 'UPLOADS') ){
			...
			// If UPLOADS isn't prepended with slash, add a slash between basedir/baseurl and UPLOADS
			$sep = '';
			if( '/' !== mb_substr( UPLOADS, 0, 1) ){
				$sep = '/';
			}
			$dirs['baseurl'] = untrailingslashit( site_url() ) . $sep . UPLOADS;
			$dirs['basedir'] = untrailingslashit( ABSPATH ) . $sep . UPLOADS;
		} else {
