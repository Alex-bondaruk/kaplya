...
final class MSMSettingsPage
{
        ...
        <div class="wrap">
            ...
            <form method="post" action="<?php echo $action_url; ?>">
                ...
                if( 'general' === $current_tab ){
                    ...
                    <?php
                }  elseif( 'troubleshoot' === $current_tab ){
	                ?>
	                <h2><?php _e( 'Troubleshooting options', 'multisite-shared-media' ); ?></h2>

	                <h3><?php _e('Reset ignore status', 'multisite-shared-media'); ?></h3>
	                <p>
		                <?php _e( 'Removes "ignore" flags from media items. This might be handy if some previously ignored items should be replicated. Doesn\'t touch physical files on disk, nor originally uploaded media items.', 'multisite-shared-media'); ?>
	                </p>
	                <button type="submit" name="reset-ignore-status" id="reset-ignore-status"
	                        class="button button-primary" value="yes" onclick="return confirm('<?php _e('Really remove all replication data?');?>');">
		                <?php echo __( 'Clear "ignore" statuses', 'multisite-shared-media' ); ?>
	                </button>
	                <?php
                }
                ?>
	...
	public function msm_save_network_options() {
		...
		if( 'troubleshoot' === $_POST['tab'] && 'yes' === $_POST['reset-ignore-status'] ){

			$current_site = get_current_blog_id();

			foreach($this->network->get_sites() as $site ){
				if ( $site->blog_id !== $current_site ) { //failsafe
					switch_to_blog( $site->blog_id );
					// delete_metadata( 'post', 0, 'msm_original_file', '', true );
					// delete_metadata( 'post', 0, 'msm_replication_info', '', true );
					delete_metadata( 'post', 0, 'msm_ignored', '', true );
				}
			}
    ...
    private function output_page_tabs( $current = 'general' ) {
	    ...
	    if ( 'yes' === $this->options['msm_do_share_media'] ) {
		    ...
		    $tabs['troubleshoot']  = __( 'Troubleshoot', 'multisite-shared-media' );
        }
